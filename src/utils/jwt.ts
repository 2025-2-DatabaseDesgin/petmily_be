...
export interface UserPayload {
  id: string; // User의 PK (id)
  username: string; // 사용자 아이디
}

export const generateAccessToken = (user: { id: bigint; username: string }): string => {
...
export const generateRefreshToken = (user: { id: bigint; username: string }): string => {
  const payload: UserPayload = {
    id: user.id.toString(),
    username: user.username
  };
